...
class pix2pix(object):
    ...
    def test_end(self, args):
        sample_files = sorted(glob('{}/example-data-Test-samples/*'.format(self.dataset_name)))
                        ...
                        with graph.as_default():
                            ...
                            with tf.Session(graph=graph, config=tf.ConfigProto(
                                ...
                                if not os.path.exists(fl[:-6] + folder):
                                    os.makedirs(fl[:-6] + folder)

                                img_name_ak1 = fl[:-6] + folder + 'ak1-1-{}'.format(sparse_count)
                                img_name_ak2 = fl[:-6] + folder + 'ak2-1-{}'.format(sparse_count)
                                img_name_ak3 = fl[:-6] + folder + 'ak3-1-{}'.format(sparse_count)
                                save_images_val(image_a,
